...
namespace Beached.ModDevTools
{
	internal class NoisePreviewerDevTool : DevTool
	{
		...
		public override void RenderTo(DevPanel panel)
		{
			...
			if (ImGui.Button("Start Watch File"))
			{
				var watcher = new FileSystemWatcher(Path.Combine(Mod.folder))
				{
		...
		{
			var path = Path.Combine(Mod.folder, biomeFilepath + ".yaml");
		...
		{
			var path = Path.Combine(Mod.folder, noiseFilepath + ".yaml");
